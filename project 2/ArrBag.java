...
public class ArrBag<T>
{
	...
	private void upSize()
	{
		ArrBag<T> newArr= new ArrBag<T>(this.size()*2);
		for (int i=0;i<this.size();i++)
		{

				newArr.add(this.get(i));
		}
		theArray=newArr.theArray;
		// MAKE NEW ARRAY 2xBIG. COPY ALL ELEMS FROM OLD ARR TO NEW ARR
		// BE SURE TO REASSIGN REF TO THE NEW 2X ARR INTO THE GLOBAL ARR NAME
	}
	...
	{
		ArrBag<T> intersection = new ArrBag<T>(Math.min(this.size(),other.size())); // REPLACE WITH YOUR CODE
		for(int i=0;i<other.size();i++){
			if(contains(other.get(i))){
				intersection.add(other.get(i));
			}
		}
		// YOUR LOOPS HERE
	...
	{
		ArrBag<T> difference = new ArrBag<T>(this.size()); // // REPLACE WITH YOUR CODE
		for(int i=0;i<this.size();i++){
	...
	{
		return union(other).difference(intersection(other)); // REPLACE WITH 1 SINGLE 1 LINER STATMENT. NO LOOPS
	}
} // END ARRBAG CLASS
